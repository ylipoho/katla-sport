...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveId: number) {
    ...
    this.hiveSectionService.setHiveSectionStatus(hiveId, true).subscribe(h => hiveSection.isDeleted = true);
  }

  onUndelete(hiveId: number) {
    var hiveSection = this.hiveSections.find(h => h.id == hiveId);
    this.hiveSectionService.setHiveSectionStatus(hiveId, false).subscribe(h => hiveSection.isDeleted = false);
  }
}
